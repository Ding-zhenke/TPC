...
├── simulation/
│   ├── ports.py             # 端口（Port, DiscretePort, FloquetPort, CablePort...）
│   ├── sources.py           # 激励源（PlaneWave, Coil, FieldSource, FarfieldSource, TimeSignal...）
│   ├── monitors.py          # 监视器（Monitor, Probe）
│   ├── boundary.py          # 边界条件（Boundary, Background, LayerStacking）
│   └── solver.py            # 求解器（Solver, FDSolver, IESolver, ParameterSweep, Optimizer...）
├── mesh/mesh.py             # 网格设置（Mesh, MeshAdaption3D, MeshSettings, MeshShapes）
...
3. **结果读取**: `result` 类独立于 `setup`，通过 `from cst_solver._result import Result` 导入
4. **配置系统**: CST 路径配置在 `cst_solver/config.py`（已加入 .gitignore）
5. **文档生成**: 运行 `python scripts/gen_docs.py` 重新生成 API 文档到 `docs/cst_solver_api.html`
...
from cst_solver._result import Result
...
- 待实现功能列表见 `docs/TODO_LIST.md`
- 项目概览见 `docs/PROJECT_OVERVIEW.md`
- 文档生成: `python scripts/gen_docs.py` → `docs/cst_solver_api.html`
