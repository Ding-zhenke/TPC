...
子模块:
    ...
    curves            — S 参数曲线判据：连续合格区间 / 谐振提取 / 谐振峰频差（V3）
                        **全仓唯一实现**：库层与 MCP 层都调它，不许另写一套规则
...
快速开始:
    ...
    >>> from tpc_toolkit.curves import find_resonances          # 只依赖标准库
    >>> peaks = find_resonances(freqs_ghz, s11_db, min_prominence_db=0.5)
